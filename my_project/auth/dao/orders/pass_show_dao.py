...
class PassShowDAO(GeneralDAO):
    _domain_type = PassShow


    def get_by_two_id(self, pass_id: int, show_id: int) -> _domain_type:
        return self._session.query(self._domain_type).filter(
            self._domain_type.pass_id == pass_id,
            self._domain_type.show_id == show_id
        ).one()

    def uptade_by_two_id(self, pass_id: int, show_id: int, in_obj: object) -> None:
    ...
    def delete_by_two_id(self, pass_id: int, show_id: int) -> None:
